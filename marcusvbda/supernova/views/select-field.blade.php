...
<div class="relative" wire:init="{{ $onInit }}('{{ $index }}')">
    ...
    @else
        <div class="relative">
            @if (!$all_is_selected)
                <select
                    wire:change="{{ $onChange }}($event.target.value,'{{ $index }}',$event.target.options[$event.target.selectedIndex].text);$event.target.value = '' "
            ...
            <div class="grid grid-cols-[repeat(auto-fill,minmax(100px,1fr))] gap-1 my-2">
                @foreach ($selected as $s)
                    <span
                        ...
                        {{ $s['label'] }}
                        <button type="button"
                            wire:click="{{ $onRemove }}('{{ $index }}', '{{ $s['value'] }}')"
